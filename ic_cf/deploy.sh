...
if [ -z "$DB_PASSWORD" ]; then
    echo "⚠️  WARNING: DATABASE_PASSWORD not found in Secret Manager"
fi
if [ -z "$OPENAI_API_KEY" ]; then
    echo "⚠️  WARNING: OPENAI_API_KEY not found in Secret Manager"
    echo "   Create with: gcloud secrets create openai_api_key_test --data-file=- --project=$PROJECT_ID"
fi

ENV_VARS="GCP_PROJECT_ID=$PROJECT_ID,INSTANCE_CONNECTION_NAME=$CLOUD_SQL_CONN,DATABASE_USER=$DB_USER,DATABASE_PASSWORD=$DB_PASSWORD,DATABASE_NAME=invoice_scanner,STORAGE_TYPE=gcs,GCS_BUCKET_NAME=$GCS_BUCKET,OPENAI_API_KEY=$OPENAI_API_KEY,PUBSUB_TOPIC_PREFIX=document-,OPENAI_MODEL=gpt-4o,GOOGLE_MODEL=gemini-2.0-flash,ANTHROPIC_MODEL=claude-3-5-sonnet-20241022,FUNCTION_LOG_LEVEL=INFO,PYTHONUNBUFFERED=1,ENVIRONMENT=cloud"
